 ...
 * rewrite replaced it with a CSV export of a few dozen lines, and the resulting
 * "nothing third-party reaches a visitor" claim is what the README's security
 * section is built on, what makes `script-src 'self'` sufficient, and what makes
 * the production `npm audit` meaningful.
 *
    ...
    it(`declares no "${field}"`, () => {
      ...
      expect(
        ...
        `"${field}" must stay empty: everything listed there reaches a visitor's browser. ` +
          `See the dependency policy in README.md before changing this.`,
      ).toEqual([]);
    });
  }
